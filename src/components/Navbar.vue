...
<template>
  <nav class="bg-blackCustom text-whiteCustom pt-10 px-5">
    <div class="flex justify-between items-center p-4 container mx-auto">
      <h1 class="text-3xl font-bold">Game Search RAWG</h1>
      <div class="flex items-center space-x-5 text-xl">
        <p v-for="({ name, href }, index) in NavLink" :key="index">
          <a :href="href" class="hover:border-b transition transform duration-200 ease-in-out">{{ name }}</a>
        </p>
        <form class="flex items-center space-x-1 bg-[#5f8bb9] rounded-md">
          <input v-model="searchQuery" type="text" placeholder="Search Game" class="py-2 px-3 focus:outline-none" />
